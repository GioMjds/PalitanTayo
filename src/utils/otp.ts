...
export const otpStorage = {
    set: (firstName: string, lastName: string, email: string, otp: string, hashedPassword: string, username: string, ttlMinutes = 15) => {
        const expiresAt = new Date(Date.now() + ttlMinutes * 60 * 1000);
        otpCache.set(email, { otp, hashedPassword, firstName, lastName, username, expiresAt });
    },
    ...
    validate: (email: string, userOtp: string) => {
        ...
        return { valid: true, data: { hashedPassword: entry.hashedPassword, firstName: entry.firstName, lastName: entry.lastName, username: entry.username } };
    }
};